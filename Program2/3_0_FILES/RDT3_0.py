...
class RDT:
    ...
    # timeout for rdt3.0
    timeout = 0.2
    # start time for rdt3.0
    start_t = None

    def __init__(self, role_S, server_S, port):
        self.network = Network3_0.NetworkLayer(role_S, server_S, port)
        self.role_S = role_S
    ...
    def role_play(self, role):
        if (role == 'server'):
            ...
            self.seq_num = (self.seq_num-1)#%2 # resend it with its actual seq_num
            self.rdt_2_1_send(self.last_p.msg_S) # then send, reincrements it back
    ...
    def rdt_2_1_send(self, msg_S, ACK='A'):
        ...
        # we dont want to resend empty NAK packets nor increment on NAKs
        if (ACK == 'A'):
            self.last_p = p
    ...
    def rdt_2_1_receive(self):
        ...
        while True:
            ...
            if (Packet.corrupt(self.byte_buffer[0:length])):
                print self.role_S + " RECEIVED CORRUPT PACKET!"
            ...
            # (or it was lost on the way back to client if using 3.0)
            # so server sends response again, but throws away the data itself
            if (ack_num != self.seq_num):
                if (self.role_S == 'server'):
                    self.role_play('client') # server is resending like the client here
                    self.byte_buffer = self.byte_buffer[length:]
                    return ret_S
                else:
                    self.byte_buffer = self.byte_buffer[length:]
                    return ret_S # client should do nothing until timeout with duplicate packet
                                 # to avoid webbing responses
    ...
    def rdt_3_0_send(self, msg_S):
        ...
        if (self.role_S == 'client'):
            self.start_t = time.time() # start the timer if client

    def rdt_3_0_receive(self):
        if (self.role_S == 'client'): # check timer if client
            cur_t = time.time()
            if (cur_t > self.start_t + self.timeout):
                self.seq_num = (self.seq_num-1) # decrement, so seq_num remains the same
                self.rdt_3_0_send(self.last_p.msg_S)
                cur_t = time.time() # reset timer if it resent due to a timeout
                print "TIMEOUT!"
